...
def test_list_flashcards_requires_auth(client: TestClient, document_id: str) -> None:
    response = client.get(f"/v1/documents/{document_id}/flashcards")
    assert response.status_code == 401


def test_list_flashcards_is_empty_by_default(authed_client: TestClient, document_id: str) -> None:
