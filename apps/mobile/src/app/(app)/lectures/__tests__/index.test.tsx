...
import LecturesScreen from "../index";
...
test("shows an empty state when there are no documents", async () => {
  ...
  expect(await screen.findByText("No lectures uploaded yet.")).toBeTruthy();
});

test("navigates to the lecture detail screen when a row is pressed", async () => {
  mockGetMe.mockResolvedValue(STUDENT_ME);
  mockListDocuments.mockResolvedValue([
    { id: "d1", title: "Intro to Systems", created_at: "2026-01-01", status: "ready" },
  ]);
  ...
  expect(router.push).toHaveBeenCalledWith("/lectures/d1");
});
...
test("hides the upload form for a non-admin", async () => {
  ...
  expect(screen.queryByText("Upload a lecture")).toBeNull();
});
...
test("shows the upload form for an admin", async () => {
  ...
  expect(await screen.findByText("Upload a lecture")).toBeTruthy();
});

test("uploads a picked PDF through requestUploadUrl -> storage -> createDocument", async () => {
  mockGetMe.mockResolvedValue(ADMIN_ME);
  mockListDocuments.mockResolvedValue([]);
  (DocumentPicker.getDocumentAsync as jest.Mock).mockResolvedValue({
    canceled: false,
    assets: [{ uri: "file:///lecture.pdf", name: "lecture.pdf", size: 1234, mimeType: "application/pdf" }],
  });
  ...
  mockCreateDocument.mockResolvedValue({ id: "d1", title: "New Lecture" });
  ...
  fireEvent.changeText(screen.UNSAFE_getByType(TextInput), "New Lecture");
